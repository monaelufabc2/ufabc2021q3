...
void ordenaBoletoValorPago(struct Boleto *, int);
double calculaValorCobrado(struct Boleto);

int main()
{
    int n, i, diaVenc, diaPag;
    ...
    for(i=0; i<n; i++)
    {
        ...
        vBol[i] = criaBoleto(banco, cedente, sacado, valor, diaVenc, diaPag, multa);
    }

    ordenaBoletoValorPago(vBol, n);
...
void ordenaBoletoValorPago(struct Boleto *v, int n)
{
    int i, j;
    struct Boleto aux;
